...
class UsersDatabase {
  Future<DocumentSnapshot<Map<String, dynamic>>> getUserInfor({required String id}) async =>
  ...
  Future<QueryDocumentSnapshot<Map<String, dynamic>>?> checkUserInDatabaseUseCase(
  ...
  Future<DocumentSnapshot<Map<String, dynamic>>?> signUserInDatabaseUseCase(
      {required String password, required String email}) async {
  ...
  Future<void> editUser({required Map<String, dynamic> user}) async {
    final doc = await usersCollection.doc(user["id"]).update(user).whenComplete(() {
      usersCollection.doc(user["id"]).get();
    });
    return doc;
  }
